...
def _get_turn_uri_port(uri: str) -> int | None:
    """Return the port component from a TURN URI if it is explicitly present."""

    if not uri.startswith(("turn:", "turns:")):
        return None
    ...
    endpoint = remainder.split("?", 1)[0].split("#", 1)[0]
    if ":" not in endpoint:
        return None
