...
class VoipConfig(Config):
    ...
    def read_config(
        ...
        if self.turn_mode not in {"coturn", "cf", "broker"}:
            raise ConfigError(
                "Invalid value for `turn_mode`",
                ("turn_mode",),
            )

        if self.turn_mode == "cf" and not self.turn_cloudflare_enabled:
            raise ConfigError(
                "`turn_cloudflare_enabled` must be true when `turn_mode` is `cf`",
                ("turn_cloudflare_enabled",),
            )

        if self.turn_mode == "broker" and not self.turn_federation_deployment:
            raise ConfigError(
                "`turn_federation_deployment` must be true when `turn_mode` is `broker`",
